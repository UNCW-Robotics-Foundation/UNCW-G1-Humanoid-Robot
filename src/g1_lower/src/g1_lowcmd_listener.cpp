...
class G1AnkleSwingSender : public rclcpp::Node {
 public:
  G1AnkleSwingSender() : Node("g1_ankle_swing_sender"), mode_machine_(0) {
    ...
    jointstate_subscriber_ = this->create_subscription<sensor_msgs::msg::JointState>(
        "lf/joint_states", 10, [this](sensor_msgs::msg::JointState::SharedPtr message) {
          JointStateListenerHandler(message);
        });
 ...
 private:
  void Control() {
    ...
    if (ms) {
      ...
      if (time_ < duration_) {
        ...
        try {
          for (int i = 0; i < G1_NUM_MOTOR; ++i) {
            motor_command_tmp.q_target.at(i) = g1JointStates[i];
          }
        }
        catch (const std::exception &e) {
          RCLCPP_ERROR(this->get_logger(), "No Jointstate or Error in Control(): %s", e.what());
        }
      }
  ...
  void JointStateListenerHandler(sensor_msgs::msg::JointState::SharedPtr message) {
    for (int i = 0; i < 29; i++) {
      g1JointStates[i] = message->position[i];
    }
  }
  ...
  jointstate_subscriber_;  // ROS2 Subscriber
