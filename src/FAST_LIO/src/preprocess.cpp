...
{
  // Unitree's /utlidar/cloud_livox_mid360 carries velodyne-style fields:
  // x, y, z, intensity (float32), ring (uint16), time (float32, per-point
  // offset). The MID360 is a non-repetitive rosette scanner, so time can
  // NOT be reconstructed from azimuth (the old spinning-lidar code here
  // produced garbage deskew offsets) — use the real time field instead.
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  // The unit of 'time' is firmware-dependent; a 10 Hz scan spans ~0.1 s,
  // so detect the unit from the span and convert to milliseconds
  // (FAST_LIO's curvature convention, see avia_handler).
  float t_min = pl_orig.points[0].time, t_max = pl_orig.points[0].time;
  for (int i = 1; i < plsize; ++i)
  {
    if (pl_orig.points[i].time < t_min) t_min = pl_orig.points[i].time;
    if (pl_orig.points[i].time > t_max) t_max = pl_orig.points[i].time;
  }
  double span = static_cast<double>(t_max) - static_cast<double>(t_min);
  double to_ms = 0.0;
  if      (span > 1e-4 && span < 1.0)  to_ms = 1e3;   // seconds
  else if (span >= 1.0 && span < 1e3)  to_ms = 1.0;   // milliseconds
  else if (span >= 1e3 && span < 1e6)  to_ms = 1e-3;  // microseconds
  else if (span >= 1e6 && span < 1e9)  to_ms = 1e-6;  // nanoseconds
  given_offset_time = (to_ms > 0.0);

  static bool unit_logged = false;
  if (!unit_logged)
  {
    printf("[mid360_handler] per-point time span %.6g => %s, deskew %s\n",
           span,
           to_ms == 1e3 ? "seconds" : to_ms == 1.0 ? "milliseconds"
           : to_ms == 1e-3 ? "microseconds" : to_ms == 1e-6 ? "nanoseconds"
           : "UNRECOGNIZED",
           given_offset_time ? "ENABLED" : "DISABLED");
    unit_logged = true;
  }
  ...
  for (int i = 0; i < plsize; ++i)
  {
    ...
    added_pt.intensity = pl_orig.points[i].intensity;
    added_pt.curvature =
        given_offset_time ? (pl_orig.points[i].time - t_min) * to_ms : 0.0;
  ...
  // Downstream code assumes points ordered by time (lidar_end_time is
  // taken from the last point).
  if (given_offset_time)
  {
